...
$$\text{SSE}(R) = \sum_{i=1}^n (Y_i - \hat{\beta}_0)^2 = (Y_i - \overline{Y})^2 = n S_Y^2$$
...
The value of $R^2_{X_j}$ is a measure of the redundancy of the predictor $X_j$. For $X_1$, this is represented in the right-hand plot of Figure \@ref(sse-partition-multicollinearity) as the proportion $\frac{D+G}{B+D+E+G}$. For $X_2$, the corresponding proportion is $\frac{D+G}{C+D+F+G}$. 
...
Generally, tolerance values below $R^2_j = .20$ are considered potentially problematic. This is by no means a precise cut-off value, but it can guide you nonetheless. Some statistics programs also report the __variance inflation factor__ (VIF), which is defined as 
