# Generalized Linear Models {#sec-ch-generalized-linear-models}
...
The key thing is that the right-hand-side of @eq-glm-link-function is a *linear function* that can, in principle, take any value between $-\infty$ and $\infty$. For variables that are bounded (e.g., non-negative variables $Y > 0$), such a linear model could provide predictions that are outside the bounds. An appropriate link function essentially makes such out-of-bounds predictions impossible. This might be easier to understand when we consider the inverse transformation: 
$$
...
Intuitively, you can think of a link function and the inverse-link function as follows: Suppose you have ten British pounds and need to convert this into US dollars. The conversion rate (at the time of writing this) is £1 = \$1.08.^[The UK financial market was rather in turmoil on 29 September 2022 after the UK government released its "mini-budget".] So if the function $g$ converts pounds ($x$) into dollars ($y$), we can write this as 
$$y = g(x) = 1.08 \times x$$
...
The test statistic used for a likelihood-ratio test is, as in Chapter @sec-ch-linear-mixed-effects-models, equal to minus two times the log likelihood ratio:
$$\begin{aligned}
...
with $k=0, 1$. Note that this is a simple case of the Binomial distribution we encountered in the Chapter @sec-ch-modeling.^[As we are only considering a single outcome, $n = 1$ and we do not need to worry about how many possible outcomes could constitute $k = 0, 1, 2, \ldots$ correct ones, which is why we don't need the $\left(\right)$ term.] Also note that as $\theta^0 = 1$, and as $k$ can only be 0 or 1, @eq-Bernoulli-distribution) simply states that 
$$\begin{aligned}
...
So the inverse of @eq-glm-logit-link-function-mu is:
\begin{equation}
...
Some examples of the relation between a single predictor and different values of $\beta_0$ and $\beta_1$ are shown in @fig-logit-link-plot.

```{r}
#| label: fig-logit-link-plot
#| fig-cap: Examples of the logistic link function for a logistic regression model with
#|   a single predictor, and different values for $\beta_0$ and $\beta_1$
library(dplyr)
library(ggplot2)
...
knitr::kable(tab, escape = FALSE, booktabs=TRUE, linesep="", digits=3, align="r")
...
@tbl-metacognition-logistic-regression-results-z-and-LR-participant-2 shows results from the same analysis applied to the data from Participant 2. Whilst the data seemed less neat than the data of Participant 1, we get rather similar results: significant and positive simple effects of confidence and contrast, as well as a significant positive interaction between them.

```{r}
#| label: tbl-metacognition-logistic-regression-results-z-and-LR-participant-2
#| tbl-cap: "Results of a logistic regression model of correct identification by confidence for Participant 2."
...
To get a better idea of the relation between confidence and accuracy, @fig-metacognition-data-and-logistic-fit-plot depicts the observed data and model predictions for the logistic regression models of @tab-metacognition-logistic-regression-results-z-and-LR-participant-1 and @tab-metacognition-logistic-regression-results-z-and-LR-participant-2. Note that the observed data are binary values indicating correct (1) or incorrect (0) responses. The model predictions however are probabilities of a correct ($Y=1$) response. Both the data and model predictions indicate more  correct responses for higher levels of contrast. For lower levels of contrast, we can see that low confidence generally implies a low probability of a correct response, whilst higher confidence a higher probability of a correct response. This patterns seems more pronounced for Participant 1. Participant 2 tends to indicate low confidence for low contrast displays, and high confidence for high contrast displays. For this participant, contrast appears a much stronger predictor of correctness. Nevertheless, @tbl-metacognition-logistic-regression-results-z-and-LR-participant-2 indicates that, for an average level of contrast, confidence has a significant unique effect on the probability of a correct response. As such, this participant appears able to recognize when they made an incorrect response.

```{r}
#| label: fig-metacognition-data-and-logistic-fit-plot
#| fig-cap: Observed and model-predicted response accuracy as a function of confidence
#|   and contrast, for two participants in the study by @rausch2016visibility on metacognition.
#| fig-width: 8
#| fig-height: 3
...
```{r}
#| label: fig-cumulative-normal-quantile-plot-Probit
#| fig-cap: The Probit link function equals the standard Normal cumulative distribution
#|   function. THe solid dark line shows the standard Normal cumulative distribution
#|   function, with arrows depicting the .05, .5, .7, and .975 quantiles.
...
The Probit link function is an S-shaped function that looks rather similar to the logistic link function. @fig-probit-link-plot shows best matching Probit link function to logistic ones. As can be seen, the functions are very similar, although the Probit function converges a little more quickly to probabilities of 0 and 1. The best match between the functions is obtained when the parameters for the Probit and logistic models are related as 
$$\beta_j^{(\text{Probit})} \approx 0.588 \times \beta_j^{(\text{logistic})}$$

```{r}
#| label: fig-probit-link-plot
#| fig-cap: A comparison of logistic and Probit link functions for a model with a single
#|   predictor, and best matching values for $\beta_0$ and $\beta_1$
#| fig-width: 6
#| fig-height: 3
library(dplyr)
library(ggplot2)
...
```{r}
#| label: metacognition-probit-regression-estimation-participant-1
library(sdamr)
data("metacognition")
...
```{r}
#| label: paul-glm-logistic-results
...
As a final example of logistic regression, I can't resist getting Paul back to the stage. We can also use logistic regression to test the hypothesis test from Chapter @sec-ch-modeling that Paul was randomly guessing, that is $H_0: p(\text{correct}) = .5$. In this case, we have a simple model without a predictor. But we can test whether the intercept equals $\beta_0 = 0$, as this value implies that $p(\text{correct}) = .5$. For this model comparison, we will compare the following models:
...
`r round(1/(1 + exp(-coefficients(mod1)[1])),3)`$. The test result of the model comparison is $G^2(`r intest[2,3]`) = `r intest[2,4]`$, $p `r format_p_text(intest[2,5])`$. Hence, in correspondence with the results from Chapter @sec-ch-modeling, we can reject the null-hypothesis that Paul was randomly guessing. <!-- You were truly amazing Paul (even after applying a generalized linear model ;-).-->
...
Apart from the choice of link function and distribution, and the resulting implications for parameter interpretation, Poisson regression is very similar to other generalized linear models. @tbl-gestures-pois-glm-results shows the results of a Poisson regression, using effect coded predictors for Context (friend = 1, professor = $-1$), Language (Catalan = 1, Korean = $-1$), and Gender (Female = 1, Male = $-1$). It should be noted that this analysis does not account for the fact that each participant contributes two data points (one for each Context). Later in this chapter we will provide results from a generalized linear mixed-effects model, which does so.
```{r}
#| label: tbl-gestures-pois-glm-results
#| tbl-cap: "Results of a Poisson regression model for the number of gestures in conversations."
...
@tbl-gestures-poisson-model-confidence-intervals shows the 95% confidence intervals, computed by the profile likelihood method, for the parameters of the Poisson regression model. As can be seen there, only the confidence intervals of Context, Gender, and the Language by Gender interaction exclude 0. This leads to similar conclusions as the test results discussed above.

```{r}
#| label: tbl-gestures-poisson-model-confidence-intervals
#| tbl-cap: "95% profile likelihood confidence intervals for the parameters of the Poisson regression model applied to the gestures data."
tab <- confint(pois_mod)
rownames(tab) <- c("Intercept","Context", "Language", "Gender","Context $\\times$ Language", "Context $\\times$ Gender", "Language $\\times$ Gender", "Context $\\times$ Language $\\times$ Gender")
knitr::kable(tab, colnames = c("lower (2.5%)", "upper (97.5%)"), escape = FALSE, booktabs=TRUE, linesep="", digits=3, align="r")
...
Overdispersion can lead to large residuals. @fig-gestures-poisson-pearson-residuals-histogram shows a histogram of the standardized Pearson residuals for the Poisson regression model estimated above. Under the null-hypothesis that the model is equal to the true model, these should be standard-Normal-distributed. The overlaid Normal density shows the expected distribution according to the model. Comparing the predicted and observed distribution, it is clear there are more large residuals than expected. 
```{r}
#| label: fig-gestures-poisson-pearson-residuals-histogram
#| fig-cap: "Hisogram of standardized Pearson residuals for the Poisson regression model"
bw <- 1
...
For the gestures data, the estimated dispersion parameter is $\hat{\phi} = `r summary(qpois_mod)$dispersion`$, which is clearly much larger than 1. Results of the quasi-Poisson regression are provided in @tbl-gestures-quasi-poisson-model-results. Note that this Table contains Wald and $F$-tests. Wald tests are justified by the asymptotic properties of quasi-likelihood estimation. The use of $F$-tests when the dispersion is estimated is recommended by @dunn2018generalized. <!-- TODO: chevk that this is appropriate -->

```{r}
#| label: tbl-gestures-quasi-poisson-model-results
#| tbl-cap: "Results of a quasi-Poisson regression model for the number of gestures in conversations."
...
@tbl-gestures-quasi-poisson-model-results indicates that none of the effects included in the model are significant. This is essentially because the standard errors of the parameters, corrected for overdispersion, are much larger than under the assumption that the Poisson distribution (without overdispersion) holds. As we will see later in this Chapter, however, the overdispersion is likely due to not properly accounting for the dependencies in the data: as participants contribute to both the friend and professor conditions, the data has repeated measures. Dealing with this properly requires the use of *generalized linear mixed-effects models*, which we will introduce in Section @sec-glmer. 
...
@tbl-rps-table shows the actions chosen by the human player (in the last half of the game) as a function of the previous actions of the AI and human player. 

```{r}
#| label: tbl-rps-table
#| tbl-cap: "Action chosen by human players in Rock-Paper-Scissors by previous actions chosen by both human and the level-1 AI player."
data("rps")
library(dplyr)
dat <- rps %>%
  group_by(id) %>%
...
data.frame(ftable(tab)) %>%
  ...
  knitr::kable(col.names = c("Previous human","Previous AI","Rock","Paper","Scissors")) %>%
...
@tbl-rps-loglm-results lists the results of all possible loglinear models for this data.
```{r}
#| label: tbl-rps-loglm-results
#| tbl-cap: "Results from all possible log-linear models for the Rock-Paper-Scissors data of @tbl-rps-table"
...
#knitr::kable(mod_comp, row.names = TRUE, col.names = c("$\\text{df}$", "$G^2$", "$p(\\gt G^2)$", "$X^2$", "$p(\\gt X^2)$"), caption="Results from all possible log-linear models for the Rock-Paper-Scissors data of @tbl-rps-table")
knitr::kable(mod_comp[,1:3], row.names = TRUE, col.names = c("$\\text{df}$", "$G^2$", "$p(\\gt G^2)$"))
```
...
Parameter estimates and Wald tests are provided in @tbl-rps-multinomial-logit.
```{r}
#| label: tbl-rps-multinomial-logit
#| tbl-cap: "Results of a baseline category multinomial logistic regression model for human current actions as a function of human and AI previous actions."
#| warning: false
#| message: false
data("rps")
dat <- rps %>%
...
tab %>%
  knitr::kable(digits=3, col.names = rep(c("$\\hat{\\beta}$","$\\text{SE}(\\hat{\\beta})$", "$z$", "$P(\\geq \\lvert z \\rvert)$"),2), align="r") %>%
...
@tbl-metacognition-glmer shows the results for a logistic mixed-effects regression model, using data from all participants of the metacognition experiment (see Section @sec-logistic-regression-metacognition). In addition to the fixed effects of Confidence, Contrast, and their interaction, the model includes participant-wise random intercepts and slopes for Confidence and Contrast. Random effects are assumed independent, and to aid estimation, Confidence and Contrast were scaled by a $Z$-transformation. Scaling predictors can be crucial when estimating generalized linear mixed-effects models.

```{r}
#| label: tbl-metacognition-glmer
#| tbl-cap: "Estimates and tests for the fixed-effects of a logistic mixed-effects model with random intercepts and slopes for Confidence and Contrast. Note: Confidence and Contrast are scaled by a Z-transformation."
#| cache: true
library(sdamr)
data("metacognition")
...
knitr::kable(tab_fix, col.names = c("$\\hat{\\beta}$", "$\\text{df}$", "$G^2$", "$p(\\geq G^2)$"), booktabs=TRUE, digits=c(3,0,2,2,3))
```

The results show significant and positive effects of Confidence and Contrast, as well as their interaction. This confirms the results obtained for models fitted to the data of the individual participants (see @tbl-metacognition-logistic-regression-results-z-and-LR-participant-1 and @tbl-metacognition-logistic-regression-results-z-and-LR-participant-2).

@tbl-gestures-glmer shows the results for a Poisson mixed-effects model for the gestures data (see Section @sec-poisson-regression-gestures). Accounting for the dependence in the data by including random intercepts for each participant, we now only find a significant effect of Context. The effects of Gender and the Gender by Language interaction are no longer significant for this model.

```{r}
#| label: tbl-gestures-glmer
#| tbl-cap: "Estimates and tests for the fixed-effects of a Poisson mixed-effects model with random intercepts for participants."
#| cache: true
library(sdamr)
data("gestures")
contrasts(gestures$context) <- c(1,-1)
