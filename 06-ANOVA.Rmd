...
knitr::kable(tab, caption = "Linear model predicting number of intrusions by a dummy predictor.", escape = FALSE, digits=3, booktabs=TRUE, linesep="")
options(opts)
```
...
colnames(tab) <- c("$\\hat{\\beta}$","$\\text{SS}$", "$\\text{df}$", "$F$", "$P(\\geq \\lvert F \\rvert)$")
knitr::kable(tab, caption = "Linear model predicting number of intrusions by three effect-coded predictors.", escape = FALSE, digits=3, booktabs=TRUE, linesep="")
options(opts)
```
