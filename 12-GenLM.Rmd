...
Intuitively, you can think of a link function and its inverse as follows: Suppose you have ten British pounds and need to convert this into US dollars. The conversion rate (at the time of writing this) is £1 = \$1.08. So if $g$ is meant to convert pounds ($y$) to dollars ($x$), we would have 
$$g(y) = 1.08 \times x$$
...
\end{aligned}$$
This means that the probability distribution for $Y$ is completely determined by $\mu_{Y|X_1, \ldots, X_m}$, and our model can focus on just this.
...
```{r metacognition-confidence-correct-plot, fig.caption="Confidence and proportion correct for two participants in the study by @rausch2016visibility on metacognition."}
library(sdamr)
data("metacognition")
...
The most common multinomial logistic regression model uses a so-called *baseline category logit* link function. 
