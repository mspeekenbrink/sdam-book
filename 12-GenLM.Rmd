...
The general linear model also consists of a random and structural part. What is new for generalized linear models is the link function.
...
When we previously discussed applying the general linear model to "tricky" data, we have suggested to transform the dependent variable and/or the predictor variables, in order to "force" the data to approximately conform to the assumptions of the GLM. Generalized linear models also use transformations, which are called link functions. However, in generalized linear models, these link functions are not applied the variables themselves, but rather to the predictions of a linear model. 
...
Take for example the inverse-log (or exponential) function above. No matter what the value of $x$, the value of $\exp(x)$ will never be below 0. It will approach 0 (from above) when $x$ approaches $-\infty$. So this inverse-link allows one to transform the unbounded predictions from a linear model into bounded predictions suitable for a non-negative continuous variable.  

Intuitively, you can think of a link function and its inverse as follows: Suppose you have ten British pounds and need to convert this into US dollars. The conversion rate (at the time of writing this) is £1 = \$1.08.^[The UK financial market was in turmoil on 29 September 2022.] So if $g$ is meant to convert pounds ($x$) to dollars ($y$), we would have 
$$y = g(x) = 1.08 \times x$$
If we were to convert our dollars back to pounds, we would need a different function, which is the inverse (it "reverses" the effect) of our function $g$. Calling this $h$, we would need
$$x = h(y) = \frac{1}{1.08} \times y \approx 0.926 \times y$$
Such transformations are also called _one-to-one-mappings_: for every amount in pounds, there is an corresponding amount in dollars, and vice versa. If I know the amount in one currency, there is only a single amount in the second currency that this refers to. The link function needs to be such a one-to-one mapping, but otherwise, we are free to choose it. The example is a linear function, but we can also choose a nonlinear function, e.g.
$$\begin{aligned}
y &= g(x) = \exp x \\
x &= h(y) = \log y 
\end{aligned}$$
...
\end{equation}
which approximately follows (again for large samples) a Chi-squared distribution with $\text{df} = 1$. This is due to a mathematical fact that if a variable $z$ follows a standard Normal distribution, then $z^2$ follows a Chi-squared distribution with $\text{df}=1$ degrees of freedom. 

Both statistics are approximate and only exact for an infinite sample size. They give entirely equivalent results, although the $z$-statistic can be used to obtain one-sided tests, whilst the $z^2$ can only be used to obtain two-sided tests. Given the additional flexibility of the $z$-statistics, this is what we will use for our Wald tests.
...
\end{aligned}$$
Note that we denote the "minus 2 log-likelihood ratio" with the symbol $G^2$, as this is the common symbol in the literature. The letter $G$ in this notation does *not* stand for our MODEL G. For large-enough samples, the sampling distribution of this statistic is approximately equal to a Chi-squared distribution:
...
\log\left(\frac{p(Y_1, \ldots, Y_n|\text{MODEL R})}{p(Y_1, \ldots, Y_n|\text{MODEL S})}\right) &= \sum_{i=1}^n \log \left( \frac{p(Y_i|\text{MODEL R})}{p(Y_i|\text{MODEL S})} \right) \\ 
...
\end{equation}
This may be easier to understand by using our shorthand notation $\pi_1 = \mu_{Y|X_1, \ldots, X_m}$:
\begin{equation}
...
So
\begin{equation}
\mu_{Y|X_1,\ldots,X_m} = \pi_1 = \frac{\exp \{ \beta_0 + \sum_{j=1}^m \beta_j X_{j} \}}{1 + \exp \left( \beta_0 + \sum_{j=1}^m \beta_j X_{j} \right)}
\end{equation}
...
As usual, the intercept $\beta_0$ reflects the model prediction when all predictors have the value $X_j = 0$. It is important to keep in mind that generalized linear models make predictions not on the scale of the dependent variable $Y$, but on the scale of the link function $g(Y)$. We can use the inverse link-function to assess the predicted value on the scale of the dependent variable. In the case of logistic regression, this is $\frac{\exp(\beta_0)}{1+\exp(\beta_0)}$. For a single-predictor model, the value $-\frac{\beta_0}{\beta_1}$ is equal to point at which $\pi_1 = 0.5$. This is the point at which the logistic curve is the steepest. The slope $\beta_1$ reflects the maximum steepness of the curve, as well as its direction. Positive values of $\beta_1$ indicate that the probability $\pi_1$ increases with the value of $x$, whilst negative values of $\beta_1$ indicate it decreases. The higher the absolute value of $\beta_1$, the more quickly the probability increases or decreases (i.e. the steeper the logistic curve).

### Parameter interpretation

In the general linear model, $\beta_j$, the slope of a predictor variable, reflects the predicted change in the dependent variable for a one-unit increase in the predictor variable $X_j$. Because the model is linear, the predicted change is the same, no matter what base value we choose for $X_j$. That means that $Y$ is predicted to change by $\beta_j$, whether we increase $X_j$ from 0 to 1, or from 100 to 101. This is not the case when we consider the predicted change in $p(Y=1|X_j)$ for different values of $X_j$. For example, for a model with $\beta_0 = 0$ and $\beta_1 = 1$, Figure \@ref(fig:logit-link-plot) shows that this probability increases more rapidly when we move from $X_1 = 0$ to $X_1 = 1$ than when we move from $X_1 = 3$ to $X_1 = 4$. 

A slope $\beta_j$ in a logistic regression model does not reflect the same effect for different values of the corresponding predictor $X_j$. But an exponential transformation of this slope does. To realise this, we can make use of the following properties of exponents:
$$\begin{aligned}
a^{(b + c)} &= a^b \times a^c \\
a^{(b \times c)} &= \left(a^{b}\right)^c = \left(a^{c}\right)^b .
\end{aligned}$$
We also need to remember that $\exp (a) = e^a$, where $e$ denotes the natural number, and $\log \left(\exp(a)\right) = a$. Using these properties, we can rewrite the odds as
$$\begin{aligned} \log\left(\frac{p(Y=1|X_1)}{1-p(Y=1|X_1)}\right) &= \beta_0 + \beta_1 X_1 \\ 
\exp \left( \log\left(\frac{p(Y=1|X_1)}{1-p(Y=1|X_1)}\right)\right) &= \exp\left(\beta_0 + \beta_1 X_1\right) \\ 
\frac{p(Y=1|X_1)}{1-p(Y=1|X_1)} &= \exp\left(\beta_0 + \beta_1 X_1\right) \\
&= \exp(\beta_0) \times \exp\left(\beta_1 \times X_1\right) \\
&= \exp(\beta_0) \times \left(\exp(\beta_1) \right)^{X_1} \end{aligned}$$
In the final line of the above derivation, you can see that every one-unit increase in $X_1$ has the effect of multiplying the odds by $\exp(\beta_1)$. So the odds at $X_1 = 1$ is the odds at $X_1 = 0$, multiplied by $\exp(\beta_1)$. Similarly, the odds at $X_1 = 2$ is the odds at $X_1 = 1$ multiplied by $\exp(\beta_1)$.
...
tab <- cbind(tab, rbind(c(NA,NA,NA),car::Anova(mod_s1, type=3)))
...
As the results show, regardless of whether we rely on the Wald or likelihood-ratio test, we find evidence for a positive interaction between confidence and contrast. We also find evidence of positive "simple effects" for confidence and contrast. As we centered the predictors, the simple effects of confidence reflects the effect of confidence for an average value of contrast. And vice versa, the simple effect of contrast reflects the effect of contrast for an average level of confidence. Taking these results together, we can conclude that -- at average levels of confidence -- higher levels of contrast result in more accurate responses. We also see that -- at average levels of contrast -- higher levels of confidence result in more accurate responses. This is thus evidence for accurate metacognition. The interaction indicates that meta-cognition might be more pronounced for higher levels of contrast (i.e., the relation between confidence and accuracy is higher for higher values of contrast). Finally, the intercept is also significant. This indicates that, for average values of confidence and contrast (such that centered $\texttt{conf}$ and $\texttt{contr}$ are both equal to 0), the predicted probability of a correct response is equal to:
$$\begin{aligned}
p(Y_i = \text{correct}) &= \frac{\exp (`r coef(mod_s1)[1]`)}{1 + \exp (`r coef(mod_s1)[1]`)} \\
&= `r exp(coef(mod_s1)[1])/(1+exp(coef(mod_s1)[1]))`\end{aligned}$$
The deviance of the model is $D_R = `r deviance(mod_s1)`$. To assess the overall model fit, we can test this by assuming the deviance (approximately) follows a Chi-Squared distribution with $n-\text{npar}(R)$ degrees of freedom (the saturated model may be taken to have a total of $n$ parameters). Hence, the test of overall model fit is $X^2(`r summary(mod_s1)$df.residual`) = `r deviance(mod_s1)`$, $p `r format_p_text(1-pchisq(deviance(mod_s1),summary(mod_s1)$df.residual))`$. As this is not significant, the model can be said to fit the data well.

Table \@ref(tab:metacognition-logistic-regression-results-z-and-LR-participant-2) shows results from the same model applied to the data from Participant 2. Whilst the data seemed less neat than the data of Participant 1, we get quite similar results, with significant and positive simple effects of confidence and contrast, as well as a positive interaction between them.

```{r metacognition-logistic-regression-results-z-and-LR-participant-2}
tab <- summary(mod_s2)$coefficients
tab <- as.data.frame(tab)
tab <- cbind(tab, rbind(c(NA,NA,NA),car::Anova(mod_s2, type=3)))
rownames(tab) <- c("Intercept","Confidence", "Contrast","Confidence $\\times$ Contrast")
colnames(tab) <- c("$\\hat{\\beta}$","$\\text{SE}(\\hat{\\beta})$", "$z$", "$p(\\geq \\lvert z \\rvert)$", "$G^2$", "$\\text{df}$", "$p(\\geq \\lvert G^2 \\rvert)$")
tab[,4] <- sapply(tab[,4], format_p)
tab[,7] <- sapply(tab[,7], format_p)
...
Figure \@ref(fig:metacognition-data-and-logistic-fit-plot) depicts the data and model predictions for the logistic regression models of Table \@ref(tab:metacognition-logistic-regression-results-z-and-LR-participant-1) and \@ref(tab:metacognition-logistic-regression-results-z-and-LR-participant-2) for the different levels of contrast used in the study. Note that the data is (correctly) depicted as binary values of either 0 or 1. The model predictions however are probabilities of a correct ($Y=1$) response as a function of confidence. Both the data and model predictions indicate more  correct responses for higher levels of contrast. For lower levels of contrast, we can see that low confidence is generally related to a low probability of a correct response, whilst high confidence is related to a higher probability of a correct response. This patterns seems more pronounced for Participant 1. Participant 2 tends to indicate low confidence for low contrast displays, and high confidence for high contrast displays. For this participant, contrast appears a much stronger predictor of correctness. Nevertheless, confidence still has a significant unique effect on the probability of a correct response. This indicates that this participant appears still able to recognize when they made an incorrect response.

```{r metacognition-data-and-logistic-fit-plot, fig.cap="Observed and model-predicted correctness of responses as a function of confidence and contrast, for two participants in the study by @rausch2016visibility on metacognition.", fig.width = 8, fig.height=3}
...
The logit link is the canonical link function for the Bernoulli and Binomial distribution. But you are not restricted to always choose the canonical link function. In principle, you can pick any (monotone and one-to-one) function $g(Y)$ that transforms variable $Y$ which is bounded between 0 and 1 to an unbounded variable with possible values between $-\infty$ and $\infty$. 

For example, a common alternative to the logit link is the so-called **Probit link function**:
$$g(y) = \text{standard-Normal-quantile}(y)$$ 
The Probit link function returns the $y$-th quantile of the standard Normal distribution. The inverse-link function can be written as
...
The Probit link function is an S-shaped function that looks rather similar to the logistic link function. Figure \@ref(fig:probit-link-plot) shows best matching Probit link function to logistic ones. As this Figure shows, the functions are very similar, although the Probit function converges a little more quickly to probabilities of 0 and 1. The best match between the functions is obtained when the parameters for the Probit and logistic models are related as 
$$\beta_j^{(\text{Probit})} \approx 0.588 \times \beta_j^{(\text{logistic})}$$

It's use may have a more natural justification than the logistic function, however. For example, suppose someone has to detect whether a visually presented line is longer than a reference. <!-- TODO -->
...
\end{equation}
with $k=0, 1, 2, \ldots$ denoting the number of occurrences, which is a non-negative integer, and ! denoting the factorial function (i.e. $x! = x \times (x-1) \times \ldots \times 1$). The parameter $\lambda$ is equal to the mean of $Y$, so $\lambda = \mu$. The variance of this distribution increases with the mean, and is actually equal to the mean, so $\text{Var}(Y) = \lambda$.
...
As an example of Poisson regression, we will consider data also analysed by @winter2021poisson. The data is from a study on the use of hand gestures by 27 participants (Catalan or Korean speakers) each conversing with either a friend (equal status person) or a professor (higher-status person). The duration of the conversation was not experimentally controlled, and the number of gestures was counted. Figure \@ref(fig:gestures-histogram) shows the numbers of gestures.
...
To analyse the data with a Poisson regression model, we need to take into account that the conversations had a different duration. If for example, conversations with friends last longer than conversations with professors, then we could expect less hand gestures in total when speaking with professors. But that could then be entirely due to the length of the conversations, and not people's tendency to use hand gestures. What we are interested in is the rate of the hand gestures (e.g. the number of gestures per minute). If we let $r$ denote the average number of gestures per minute, and $t_i$ the duration (in minutes) of a conversation $i$, then the expected number of gestures in conversation $i$ would be $\mu_i = r \times t_i$. A Poisson regression model predicts $\log(\mu)$, which we can write in terms of the rate $r$ and duration $t_i$ as:
$$\begin{aligned} \mu &= r \times t_i \\ 
\log (\mu_i) &= \log\left(r \times t_i\right) \\
&= \log(r) + \log(t_i)
\end{aligned}$$
In the study, we are interested in how social context and language affect the rate of hand gestures. So ideally we would like to model the (log) rate $\log(r)$, and not the expected number of gestures $\log(\mu_i)$. We can do this by replacing $\log(r)$ above with the linear model predictions. We then have the following model for the expected counts: 
$$\log (\mu_i) = \log(t_i) + \beta_0 + \sum_{j=1}^m \beta_j X_{j,i}$$
Note that the term $\log(t_i)$ above has no slope. You can think of $\log(t_i)$ as a predictor with a fixed slope of 1. Such a term added to a model (without needing to estimate a corresponding parameter) is also called an **offset**.

Table \@ref(tab:gestures-pois-glm-results) shows the results of a Poisson regression, using effect coded predictors for Context (friend = 1, professor = $-1$), Language (Catalan = 1, Korean = $-1$), and Gender (Female = 1, Male = $-1$). It should be noted that this analysis does not account for the fact that each participant contributes two data points (one for each Context). Later in this chapter we will provide results from a generalized linear mixed-effects model, which does so.
...
```
As before, we provide results for both the Wald and likelihood-ratio tests, whilst you would normally just report one of these. Whilst the results are very similar, given the relatively small size of the dataset, the likelihood ratio test is recommended. The results indicate a significant and positive effect of Context, indicating more gestures in conversations with friends. There is also a significant and positive effect of Gender, indicating more gestures by females compared to males. Finally, there is a significant and negative interaction between Language and Gender. This indicates that the difference between females and males is reduced for Catalan speakers as compared to Korean speakers.
...
In a Poisson distribution, the variance is identical to the mean. In real data, the variance is often larger than the mean. When the variance is larger than theoretically expected, this is called **overdispersion**. Underdispersion is also possible, but much less common. Overdispersion does not bias the estimated parameters $\hat{\beta}$, however, the standard errors of these estimates are higher than 

There are ways to account for overdispersion in generalized linear models. In quasi-Poisson regression, it is assumed that the variance is not equal to the mean $\mu$, but scaled by a dispersion parameter $\phi$, such that $\text{Var}(Y) = \phi \times \mu$. If $\phi = 1$, then there is no overdispersion. If $\phi > 1$, there is overdispersion. By estimating the dispersion parameter, quasi-Poisson regression allows to account for overdispersion and can compute more accurate standard errors. For the gestures data, the estimated dispersion parameter is $\hat{\phi} = `r summary(qpois_mod)$dispersion`$, which is clearly much larger than 1. Results of the quasi-Poisson regression are provided in Table \@ref(tab:gestures-quasi-poisson-model-results). Note that this Table contains Wald and $F$-tests. The use of $F$-tests when the dispersion is estimated is recommended by @dunn2018generalized.
...
rownames(tab) <- c("Intercept","Context", "Language", "Gender","Context $\\times$ Language", "Context $\\times$ Gender", "Language $\\times$ Gender", "Context $\\times$ Language $\\times$ Gender")
colnames(tab) <- c("$\\hat{\\beta}$","$\\text{SE}(\\hat{\\beta})$", "$z$", "$p(\\geq \\lvert z \\rvert)$", "$F$", "$\\text{df}$", "$p(\\geq \\lvert F \\rvert)$")
tab[,4] <- sapply(tab[,4], format_p)
tab[,7] <- sapply(tab[,7], format_p)
...
Log-linear models are used to analyse dependency relations in multivariate categorical data. Such data can be expressed in the form of *multiway contingency tables* that contain counts for combinations of categorical classifications. For example, we might be interested in whether there is a relation between the newspaper people read and the political party they vote for. To investigate this, we can count how often each combination of newspaper and political party occurs in a random sample of surveyed people.

Let $\mu_{i,j}$ denote the expected frequency of combination where the first categorical variable has value $A = i$ and the second the value $B = j$. The probability that one randomly drawn observation has the values equal to this combination is $\pi_{i,j} = P(A = i, B = j)$. For a total of $n$ independent observations, the expected frequency is then: 
...
and we can write logarithm of the expected counts as:
$$\begin{aligned} \log \mu_{ij} &= \log(n) + \log(\pi_{i,\cdot}) + \log(\pi_{\cdot,j}) \\ &= \lambda + \lambda_i^{(A)} + \lambda_j^{(B)} \end{aligned}$$
If independence does not hold, then the above equation is incorrect, and we would need an additional term to correct for this. In that case, we can write
...
Log-linear models use such constructions to test for independence, in which case the "correction terms" above would all be $\lambda_{i,j}^{(A,B)} = 0$. Log-linear models are similar in spirit to a factorial ANOVA, in the sense that the counts are decomposed into main effects, two-way interactions, three-way interactions, etc. For a two-way contingency table, the counts $\mu_{i,j}$ can always be decomposed exactly as
...
data.frame(ftable(tab)) %>%
  ...
  knitr::kable(caption = "Action chosen by human players in Rock-Paper-Scissors by previous actions chosen by both human and AI player.", col.names = c("Previous human","Previous AI","Rock","Paper","Scissors")) %>%
  add_header_above(header=c(" " = 2, "Current human action" = 3))
```

There are 9 possible models that can be used for this three-way contingency table. One possible model i includes only main effects for the previous human action (H), the previous AI action (A) and the current human action (C). Such a model can be denoted as $(H, A, C)$, where elements separated by a comma are assumed independent:
$$\begin{aligned}(H,A,C)&& \log(\mu_{i,j,k}) = \lambda + \lambda_i^{(H)} + \lambda_j^{(A)} + \lambda_k^{(C)}\end{aligned}$$
If humans played the optimal strategy, then we would expect a dependency between the previous human action and the current human action, but these to be independent from the previous AI action. This corresponds to a model with main effects for $H$, $A$, and $C$, and a two way interaction between $H$ and $C$. This model can be denoted as $(HC,A)$, to indicate dependence between $H$ and $C$, and independence of $A$:
...
Instead of the Z-test for the individual coefficients in Table , we may wonder whether a predictor has "any effect" on the dependent variable. For example, if the AI's previous action has no effect on the human's current action, then that would mean that the null-hypothesis $$H_0: \beta_{R, AR} = \beta_{R, AS} = \beta_{S, AR} = \beta_{S, AS} = 0$$ is true. We can test this hypothesis by a model comparison, comparing the full model to one where we fix these parameters to 0 (effectively removing the AI previous action predictor from the model).
...
The results show significant and positive effects of Confidence and Contrast, as well as their interaction. This confirms the results obtained for models fitted to the data of the individual participants (see Table \@ref(metacognition-logistic-regression-results-z-and-LR-participant-1) and \@ref(metacognition-logistic-regression-results-z-and-LR-participant-2)).

Table \@ref(tab:gestures-glmer) shows the results for a Poisson mixed-effects model for the gestures data. Accounting for the dependence in the data by including random intercepts for each participant, we now only find a significant effect of Context. The effects of Gender and the Gender by Language interaction are no longer significant in this model.
...
3. If there are repeated measures or other dependencies between data points, consider adding random effects to account for this.

4. Estimate and assess the fit of the model. Overdispersion can be an issue.

5. Report the results.

## Further reading

This chapter can only provide an introduction to generalized linear models. Whole books have been written on the topic. If you want to read more about generalized linear models, @agresti2018introduction provides a good and relatively readable introduction. @dobson2018introduction provides a more succinct and technical text. @hosmer2013applied offer a readable and practical book on logistic regression. @wickens2014multiway provides a very extensive book on loglinear models.