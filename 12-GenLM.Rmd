...
\end{equation}
This may be easier to understand by replacing $\mu_{Y|X_1, \ldots, X_m} = = p(Y=1|X_1, \ldots, X_m)$ with the shorthand $\pi_1$:
\begin{equation}
...
Table \@ref(tab:metacognition-logistic-regression-results-z-participant-1) shows the results for Participant 1. Each participant completed 378 trials, so the number of observations in this subset of data equals $n=378$. For didactic purposes, we present results from both the $z$ and likelihood-ratio tests (you would usually just provide one of these). 
...
colnames(tab) <- c("$\\hat{\\beta}$","$\\text{SE}(\\hat{\\beta})$", "$z$", "$p(\\geq \\lvert z \\rvert)$", "$X^2$", "$\\text{df}$", "$p(\\geq \\lvert X^2 \\rvert)$")
