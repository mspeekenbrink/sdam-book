...
The general linear model also consists of a random and structural part. What is new here is the link function.
...
$$h(g(x)) = x$$
Whilst the choice of a link function is essentially up to the analyst, it is common to use a **canonical link function**. Canonical link functions are, roughly put, link functions that are the most natural way to express $\mu_{Y|X_1,\ldots,X_m}$ with a linear function.<!-- TODO --> 
...
\end{equation}
approximately follows a standard Normal distribution. Note that this $z$ statistic is similar to the $t$-statistic for the General Linear Model. What is missing however are the degrees of freedom. For large samples, as the number of observations $n$ approaches infinity, the $t$-distribution converges to the standard Normal distribution. For large-enough $n$, the difference between the $t$-distribution and the Normal distribution so small that there is little difference when using one or the other to compute things like $p$-values (but technically, the use of a Normal distribution is only truly correct when the number of observations is infinite).
...
The test statistic used for a likelihood-ratio test is, as in Chapter XXX, equal to minus two times the log likelihood ratio:
$$\begin{aligned}
...
\end{equation}
where $\hat{V}(Y_i|X_{1,i}, \ldots, X_{m,i})$ denotes the (estimated) variance of $Y_i$ for cases with predictor values $X_{1,i}, \ldots, X_{m,i}$, and $h_i$ is the *leverage* value.]
...
&= -2 \log \left(p(\text{DATA}|\text{MODEL R})/p(\text{DATA}|\text{MODEL S})\right) - (-2 \log \left( p(\text{DATA}|\text{MODEL G})/p(\text{DATA}|\text{MODEL S})) \right) \\
&= D_R - D_G
\end{aligned}$$
...
Figure \@ref(fig:metacognition-data-and-logistic-fit-plot) depicts the data and model predictions.
...
So, the Probit link function returns the $y$-th quantile of the standard Normal distribution. The inverse-link function can be written as
...
The Probit link function has a shape that is quite similar to the logistic link function. It is of theoretical interest however, as it may have a more natural interpretation than the logistic function. For example, suppose someone has to detect
```{r}
library(sdamr)
data("metacognition")
...
Poisson regression is useful when the total number of possible occurrences of an event in not given. For example, suppose you give participants one hour solve as many algebra problems as they. Some participants may be able to (on average) solve a problem every minute, whilst others may need longer (e.g. 5 minutes per problem, on average). And some participants may even solve problems in less than a minute. 
