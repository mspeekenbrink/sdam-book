...
```{r coin-flip-relative-frequency-multiple, echo=FALSE,fig.cap=paste("Relative frequency of heads when flipping a coin",n_sim,"times for",n_flip,"times"), cache=TRUE}
...
When considering the probability of a sequence of events (e.g. a consecutive run of 8 correct predictions), it can be useful to draw all possible outcomes in the form of a tree.
...
An __efficient__ estimator is an estimator which is both unbiased _and_ the most consistent possible. Such an estimator is, in some sense, the Holy Grail of statistical estimation. 
