...
ggplot(data.frame(
  ...
  lr = c(tab[1:8,5],tab[1:8,5]),
  p = c(tab[1:8,2] + c(tab[15:9,2],0),c(dbinom(0:6,size=14,prob=.9) + dbinom(14:8,size=14,prob=.9), dbinom(7,size=14,prob=.9)))
),aes(x=lr,y=p)) + geom_col(width=.005) + xlab("likelihood ratio") + ylab("P(likelihood ratio | MODEL R)") + facet_wrap(~theta, labeller=label_parsed) + geom_vline(xintercept=sum(tab[c(3,4),5])/2,lty=2)

```

For $\alpha = .05$, the critical value is `dbinom(2,14,.5)/dbinom(2,2,12/14)`, so again, we can reject 
