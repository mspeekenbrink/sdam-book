# Path models (SEM 1) {#sec-ch-SEM-path-models}
...
Up to now, we have mostly considered models with a single dependent variable, allowing the value of this variable to depend on multiple predictors or independent variables. Although flexible, this can still be restrictive. There are situations where we would like to simultaneously model the inter-relations within a set of variables. We saw an example of this when we considered *mediation* (see Chapter @sec-ch-moderation-mediation), where we were interested in how an independent variable affects a mediator variable, and how this mediator variable subsequently affects a dependent variable. Such a *causal chain* can not be directly modelled in a single regression model. Applying the causal steps approach, we needed to estimate three separate models, with inference being based on a comparison of the parameters between these models.

In this chapter, and the next, we will consider how to account for the relations between multiple dependent variables. We will focus on a class of models called **Structural Equation Models** (SEM). As we will see, this class incorporates a wide variety of models, including many of those we have discussed before. Structural equation models also allow us to incorporate **latent variables**, which are variables that are not directly observed, but whose values can be inferred (or measured) from their relations with observed variables. Such latent variable models are the focus of Chapter @sec-ch-SEM-latent-variable-models. Here, we will focus on models which contain only observed variables. These models are commonly referred to as **path models**.

Path models were introduced by @wright1920relative, who used regression equations to define direct and indirect effects of observed variables onto others in population genetics, and applied causal interpretations to these effects. @fig-wright-path-model-image shows one of the first illustrations of a path model by Sewell Wright, to depict genetic relations between generations of guinea pigs.

|[Graphical representation of a path model by Sewell Wright. Source: [OpenMx](https://openmx.ssri.psu.edu/sites/default/files/Wright20aFigure5_1.jpg)](https://openmx.ssri.psu.edu/sites/default/files/Wright20aFigure5_1.jpg){#fig-wright-path-model-image}

```{r}
#| label: wright-path-model-image
#| eval: false
#| fig-cap: 'Graphical representation of a path model by Sewell Wright. Source: [OpenMx
#|   ](https://openmx.ssri.psu.edu/sites/default/files/Wright20aFigure5_1.jpg)'
...
```{r}
#| label: fig-SEM-simple-regression-structural-plot
#| fig-cap: Graphical representation of a simple regression model
#| fig-width: !expr 1.2 * 4
#| fig-height: !expr 1.2 * 1
#| out-width: 80%
library(sdamr)
library(lavaan)
library(semPlot)
...
Graphical depictions of SEM models, such as @fig-SEM-multiple-regression-speeddate-plot, contain a wealth of information about the assumed relations between the variables and the estimated parameters. However, sometimes we need more precise information, as well as parameter tests, which can be provided in a table such as @tbl-sem-table-multiple-regression-speeddate. This table lists all the parameters of the path model in order (regression slopes, intercepts, residual variances, residual covariances). For estimated parameters, the table also shows the standard error of the estimate, and the results of Wald tests of the null-hypothesis that the parameter equals 0. For fixed parameters, only the value of the parameter is shown. Fixed parameters can be computed, rather than estimated, and hence they do not have a standard error. The final part of the table contains so-called *fit indices*. We will discuss these later on.

```{r}
#| label: tbl-sem-table-multiple-regression-speeddate
#| tbl-cap: "Results of a SEM analysis of a multiple regression model"
...
Alternatively, we might assume that motivation causes both intention and behaviour, but that intention and behaviour are conditionally independent. This is also called a **common cause model**. This model is depicted in @fig-path-common-cause-legacy-motive. Note the broken bi-directional arrow between $\texttt{donation}$ and $\texttt{intention}$ with a value of 0. This indicates that the residual covariance between these variables is fixed to 0.

```{r}
#| label: fig-path-common-cause-legacy-motive
#| fig-cap: A common cause model
#| fig-width: !expr 0.8 * 6
#| fig-height: !expr 0.8 * 4
#| out-width: 80%
#summary(fmod1)
...
We have introduced the multivariate Normal distribution in Chapter @sec-ch-linear-mixed-effects-models. A **multivariate Normal distribution** is a distribution over _vectors_ of values. An example for a vector of two variables, e.g. $[X_1,Y_1]$, is shown in @fig-bivariate-normal-distribution-interactive-plot.
...
```{r}
#| label: fig-bivariate-normal-distribution-interactive-plot
#| cache: true
#| fig-cap: A multivariate Normal distribution for two variables. You can rotate the
#|   plot with a mouse by clicking and moving the mouse.
#| fig-align: center
...
#### Variance-Covariance algebra {#sec-SEM-path-models-variance-covariance-algebra}
...
A test of overall model fit is similar to that used for generalized linear models (see Section @sec-generalized-linear-models-overall-model-fit), which is based on likelihood-ratio test comparing the model under consideration (MODEL M) to a saturated MODEL S:
$$\begin{aligned}
...
With these technical details behind us, we can finally assess the fit of the three models proposed in Section @sec-sem-path-model-mediation. These were the full mediation, partial mediation, and the common cause model. The overall fit tests and fit measures of these models are provided in @tbl-sem-medation-fit-measures.

```{r}
#| label: tbl-sem-medation-fit-measures
#| tbl-cap: "Fit tests and measures for three candidate path models for relating legacy motive, intention, and donations."
...
knitr::kable(tab, digits=c(0,3,1,0,3,3), align="r") %>%
...
Perhaps surprisingly, apart from the effect of age on legacy and income, all the paths in the model were significant. @tbl-sem-path-mod-complex-results shows the results in more detail. Unfortunately, the model fit indices indicate that this model does not provide a satisfactory account of all the relations in the data. 

```{r}
#| label: tbl-sem-path-mod-complex-results
#| tbl-cap: "Results of a SEM analysis of a multiple regression model"
#| 
# if(knitr::is_html_output()) {
...
#   semTable::semTable(fmod_complex, type="html", paramSets = c("slopes","fits"), fits = c("chisq", "cfi", "srmr", "rmsea","aic", "bic"))
#   cat("</table>")
# } else {
#   semTable::semTable(fmod_complex, type="latex", table.float = TRUE, fits = c("chisq", "cfi", "srmr", "rmsea","aic", "bic"), caption = "(\\#ttab:tab:sem_path_mod_complex_results) Results of an initial full path model for the legacy motive data.")
# }

semoutput::sem_paths(fmod_complex) |> 
  gt::cols_hide(stars)

# cat(": Results of a SEM analysis of a multiple regression model {tbl-sem-path-mod-complex-results}")
```

@tbl-complex-path-legacy-motive-mi shows the modification indices for fixed-to-zero parameters in this model, ordered by magnitude and ignoring parameters for which the modification index is smaller than 2. 


```{r}
#| label: tbl-complex-path-legacy-motive-mi
#| tbl-cap: "Modification indices and expected change in parameter values for fixed-to-zero parameters."
fmod_complex_mi <- lavaan::modificationindices(fmod_complex, sort=TRUE, standardized = FALSE, minimum.value = 2)
fmod_complex_mi |>
...
#   semTable::semTable(fmod_complex2, type="html", paramSets = c("slopes","fits"), fits = c("chisq", "cfi", "srmr", "rmsea","aic", "bic"))
#   cat("</table>")
# } else {
#   semTable::semTable(fmod_complex2, type="latex", table.float = TRUE, fits = c("chisq", "cfi", "srmr", "rmsea","aic", "bic"), caption = "(\\#ttab:tab:sem_path_mod_complex_results2) Results of a second full path model for the legacy motive data.")
# }

semoutput::sem_paths(fmod_complex2) |> 
  gt::cols_hide(stars)

#cat(": Results of a second full path model for the legacy motive data. Only results for regression parameters and selected model fit indices are included {tbl-sem-path-mod-complex-results2}")
```
