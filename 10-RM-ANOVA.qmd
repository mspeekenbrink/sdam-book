# Repeated-measures ANOVA {#sec-ch-RM-ANOVA}
...
<!-- As another example of dependency between model residuals, let's go back to the simplest version of the GLM, the model with just an intercept that was the focus of Chapter \@ref(sec-ch-simple-GLM):
\begin{equation}
...
The design of the study is an example of a 2 (Version: Identical, Variant) by 3 (Presentation: Alone, Different, or Similar) design. The first factor (Version) varied between people, and the second (Presentation) within people (each participant rated a face in each of the three conditions). The rated attractiveness of the faces in the different conditions are provided in @fig-cheerleader-raincloud-plot.

```{r}
#| label: fig-cheerleader-raincloud-plot
#| fig-cap: Attractiveness ratings for photo's of faces when presented alone, as part
#|   of a group of dissimilar faces (Different), or as part of a group of similar faces
#|   (Similar), which are either identical (Identical) or different photos (Variant)
#|   of the same face.
#| fig-width: 6
#| fig-height: 3
library(dplyr)
...
```{r}
#| label: helmert-contrast-cheerleader
...
If we were to (wrongly!) treat all ratings as independent, and analyse the data with a regular oneway ANOVA, we would obtain the results in @tbl-cheerleader-oneway-ANOVA-results. As you can see, this analysis indicates there is no effect of presenting a face alone or in a group.

```{r}
#| label: tbl-cheerleader-oneway-ANOVA-results
#| tbl-cap: "Linear model predicting attractiveness ratings in the three conditions, wrongly assuming independence."
...
knitr::kable(tab, escape = FALSE, digits=c(3,0,0,2,3), booktabs=TRUE, linesep="")
options(opts)
```
...
dat %>%
  ...
  knitr::kable(row.names = FALSE, col.names = c("Participant","Alone", "Different", "Similar", "Average", "D-A", "S-A"), digits=2)  
...
As an example, let's compute the within-subjects composite scores for participant 36 in @tbl-cheerleader-self-attractiveness-table. For contrast $d_1$, we compute
$$\begin{aligned}
...
\end{aligned}$$
@tbl-cheerleader-self-composite-table shows the resulting values for other participants as well.
```{r}
#| label: tbl-cheerleader-self-composite-table
#| tbl-cap: "Attractiveness ratings in the Alone, Different, and Similar condition for 10 participants in the Variant condition. Also shown are three within-subjects composite scores."
...
dat %>%
  ...
  mutate(Alone = format(Alone, digits=2, nsmall=2),
         Different = format(Different, digits=2, nsmall=2),
         Similar = format(Similar, digits=2, nsmall=2)) %>%
  knitr::kable(row.names = FALSE, col.names = c("Participant","Alone", "Different", "Similar", "$W_0$", "$W_1$", "$W_2$"), digits=c(0,2,2,2,2,2,2))  

```

```{r}
#| label: compute-aovs-2
wdat <- dat %>%
  group_by(Participant) %>%
  tidyr::pivot_wider(id_cols = Participant, names_from = Item, values_from = Response) %>%
  ungroup() %>%
...
Comparing the results to the oneway ANOVA of @tbl-cheerleader-oneway-ANOVA-results shows that we get the same parameter estimates (after rescaling), but different test results. That is because the analyses using within-subjects composite scores consider the effects of the within-subjects effects regardless of whether someone is generally attracted to that face or not. In the oneway ANOVA, a substantial part of the variation within the conditions is due to individual differences in how attracted people are to a particular face. Some participants gave higher attractiveness ratings (regardless of condition) than other participants (see @tbl-cheerleader-self-attractiveness-table). This relatively high variability within the conditions can't be "explained" in the oneway ANOVA. This leads to relatively large errors, and with that relatively low power for the tests of condition. If we know that one participant is relatively more attracted to a face than other participants, we could use that knowledge to make better predictions of their attractiveness ratings. This is, roughly, what repeated-measures ANOVA is about: separating individual differences in average scores (over all within-subjects conditions) from effects of the within-subjects manipulations. 
...
Note that this is equal to the intercept of the oneway ANOVA in @tbl-cheerleader-oneway-ANOVA-results. As in that model, a test of the hypothesis that the intercept equals 0 is not overly interesting. But when we consider between-subjects effects, the composite score $W_0$ is crucial. 
...
tab %>%
  knitr::kable(col.names=c("$\\hat{\\beta}$","$\\text{SSE}(R)$", "$\\text{SSE}(G)$", "$\\text{SSR}$", "$\\text{df}_1$", "$\\text{df}_2$", "$F$"), escape=FALSE, booktabs=TRUE, digits=c(2,0,0,0,0,0,2))
options(opts)
```

```{r}
#| label: tbl-cheerleader-oneway-ANOVA-results-2
#| tbl-cap: "Linear model predicting attractiveness ratings in the three conditions, wrongly assuming independence."
modg <- lm(Response ~ Item, data=dat)
opts <- options()
options(knitr.kable.NA = "")
...
opts <- options()
options(knitr.kable.NA = "")
knitr::kable(atab, digits=c(3,0,0,2,3), escape = FALSE, booktabs = TRUE, linesep="")
options(opts)
```
...
The crux of repeated-measures ANOVA is that we can separate between-subjects effects (e.g. individual differences) from within-subjects effects. This means we can separate the total variance in $Y_{j,i}$ scores into parts that are due to between-subjects effects (e.g. individual differences) and within-subjects effects (e.g. differences due to within-subjects manipulations). This often provides more powerful tests, at least for the within-subjects effects. A graphical representation of the partitioning of the total variation into the between and within parts is provided in @fig-partitioning-variance-rm-anova.

<!-- TODO: why is svg not working well with fonts? -->

```{tikz}
%| label: fig-partitioning-variance-rm-anova
%| fig-cap: Partitioning the total Sum of Squares in a oneway repeated-measures ANOVA.
...
To make this less abstract, let's apply this idea now. @tbl-cheerleader-all-composite-table shows the values of the composite variables (computed in the same way as before), as well as the contrast-coded predictor ($X$) which codes for Version.
```{r}
#| label: tbl-cheerleader-all-composite-table
#| tbl-cap: "Attractiveness ratings in the Alone, Different, and Similar condition for 5 participants in the Identical and 5 participants in the Variant condition. Also shown are three within-subjects composite scores."
#options(pillar.sigfig = 2)
dat %>%
  group_by(Participant) %>%
  ...
  mutate(Alone = format(Alone, digits=2, nsmall=2),
         Different = format(Different, digits=2, nsmall=2),
         Similar = format(Similar, digits=2, nsmall=2)) %>%
  knitr::kable(row.names = FALSE, col.names = c("Participant","Version","Alone", "Different", "Similar", "$X_1$", "$W_0$", "$W_1$", "$W_2$"), digits=c(0,2,2,2,2,2,2,2,2))
...
```{r}
#| label: tbl-cheerleader-W0-ANOVA-table
#| tbl-cap: "Results of the model for $W_0$, testing for between-subjects effects."
...
knitr::kable(tab0, digits=c(3,0,0,2,3), escape = FALSE, booktabs = TRUE, linesep="")
options(opts)
```
...
knitr::kable(tab1, digits=c(3,2,0,2,3), escape = FALSE, booktabs = TRUE, linesep="")
options(opts)
```

The procedure for $W_2$ is the same as for $W_1$. The results of the two model comparisons are provided in @tbl-cheerleader-W2-ANOVA-table. Here, we see that the intercept is not significant. Hence, there is no evidence of a main effect of $d_2$, which compares the Different presentation to the Similar presentation. But the slope of $X_1$, which reflects the $d_2$ by $c_1$ interaction, is significant and estimated as negative. There is thus evidence that the difference between the Different and Similar presentation varies over the two versions of the Similar presentation.
```{r}
#| label: tbl-cheerleader-W2-ANOVA-table
#| tbl-cap: "Results of the model for $W_2$, testing for the within-subjects difference between the Different and Similar vs Alone presentations, and the interaction of this effect with Version."
...
knitr::kable(tab2, digits=c(3,2,0,2,3), escape = FALSE, booktabs = TRUE, linesep="")
options(opts)
```
...
The collated results of all analyses are provided in @tbl-cheerleader-rm-anova-collated-results-table. This table is separated in a between-subjects part, and a within-subjects part. Relevant omnibus tests and tests for specific contrasts are provided for each. The final row shows the Total Sum of Squares, which is the sum over the between-subjects and within-subjects Sum of Squares. This shows how the Total Sum of Squares is partitioned in the different (between- and within-subjects) effects. 
```{r}
#| label: tbl-cheerleader-rm-anova-collated-results-table
#| tbl-cap: "Complete results of the 2 (Version: Identical, Variant) by 3 (Presentation: Alone, Different, Similar) ANOVA with repeated-measures on the last factor."
atab <- rbind(
  c(NA, sum(tab0[,2]), sum(tab0[,3]), NA, NA),
  tab0,
  c(NA, sum(c(tab1[,2],tab2[,2])), sum(c(tab1[,3],tab2[,3])), NA, NA),
...
opts <- options()
options(knitr.kable.NA = "")
knitr::kable(atab, digits=c(3,2,0,2,3), escape = FALSE, booktabs = TRUE, linesep="")
options(opts)
```

A graphical overview of how the variation in $Y$ is partitioned in this analysis is provided in @fig-partitioning-variance-rm-anova-full.

```{tikz}
%| label: fig-partitioning-variance-rm-anova-full
%| fig-cap: Partitioning the total Sum of Squares in a 2 by 3 mixed repeated-measures
%|   ANOVA.
%| dev-args: !expr list(pointsize = 12)
%| fig-format: png
  \usetikzlibrary{arrows}
...
\end{equation}
Here, I'm using a somewhat different notation than in @eq-glm-factorial-ANOVA-model. Here, $\pi_i$ stands for the person-effect of participant $i$:
$$\pi_i = \mu_{i,\cdot} - \mu$$
...
5. Interpret and report the results. When reporting the results, make sure that you include all relevant statistics. For example, one way to write the results of the analysis of @tbl-cheerleader-rm-anova-collated-results-table, is as follows:
