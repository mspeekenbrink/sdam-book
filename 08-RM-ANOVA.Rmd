...
rownames(tab) <- c("Intercept","Condition","$\\quad X_1$ (Different)","$\\quad X_2$ (Same)", "Error")
colnames(tab) <- c("$\\hat{\\beta}$","$\\text{SS}$", "$\\text{df}$", "$F$", "$P(\\geq \\lvert F \\rvert)$")
knitr::kable(tab, caption = "Linear model predicting attractiveness ratings in the three conditions, wrongly assuming independence. Effect-coding was used for Condition, with the first contrast representing the effect of Different, and the second the effect of Same.", escape = FALSE, digits=3, booktabs=TRUE, linesep="")
options(opts)
```
...
As the cheerleader effect predicts that faces presented as part of a group will be rated as more attractive, and that this effect will be larger if there is more variety in the faces within the group, a reasonable set of contrast codes is:
```{r}
...
knitr::kable(tab, booktabs=TRUE, linesep="")
```
