...
if(knitr::is_html_output()) {
  ...
  semTable::semTable(fregmod, type="html", fits = c("chisq", "nfi", "tli", "srmr", "rmsea"))
  cat("</table>")
} else {
...
\begin{equation}
\text{npar}(S) = \frac{P \times (P - 1)}{2} + 2 \times P = P \times P
...
The **normed fit index** (NFI) is computed from the overall fit tests of MODEL M and MODEL B as a ratio similar to that of the $R^2$ measure for the GLM:
\begin{equation}
...
The **Tucker-Lewis index** (TLI) is another common fit index comparing a model to a baseline=. Like the NFI, it uses the overall model fit statistics $\hat{\chi}^2_M$ and $\hat{\chi}^2_B$, but considers how these deviate from their expected values (under the null-hypothesis that the each model is equal to MODEL S). For a Chi-squared distribution, the expected value (the mean) is equal to the degrees of freedom. Hence, the expected values are $\text{df}_M = \text{npar}(S) - \text{npar}(M)$ and $\text{df}_B = \text{npar}(S) - \text{npar}(B)$ respectively. The TLI (also called the non-normed fit index, NNFI) is computed as:
\begin{equation}
\text{TLI} = 1 - \frac{(\hat{\chi}^2_M - \text{df}_M)/(\hat{\chi}^2_B - \text{df}_B)}{\text{df}_M/\text{df}_B}
(\#eq:sem-TLI-definition)
\end{equation}
The TLI is often interpreted as a measure between 0 (bad) and 1 (good), although it technically can have values below below 0 or above 1. In any case, a value larger than .95 can be considered satisfactory [@hu1999cutoff]. 
...
\begin{equation}
\text{SRMR} = \sqrt{\frac{2 \sum_{i=2}^T \sum_{j=1}^i \left(\frac{s_{ij} - \sigma_{ij}(\theta)}{s_{ii} \times s_{jj}}\right)^2}{T \times (T+1)}}
...
\begin{equation}
\text{AIC} = -2 \log p(\text{DATA}|\text{MODEL}) + 2 \times \text{npar}(M)
(\#eq:AIC-definition)
\end{equation}
...
\begin{equation}
\text{BIC} = -2 \log p(\text{DATA}|\text{MODEL}) + \text{npar}(M) \times \log (n)
(\#eq:BIC-definition)
\end{equation}
...
if(knitr::is_html_output()) {
  ...
  semTable::semTable(fmod_complex, type="html", paramSets = c("slopes","fits"), fits = c("chisq", "nfi", "tli", "srmr", "rmsea"))
  cat("</table>")
} else {
