# Latent variable models (SEM 2) {#sec-ch-SEM-latent-variable-models}
...
\end{aligned}$$
Using variance-covariance algebra (Section @sec-SEM-path-models-variance-covariance-algebra), this implies that
\begin{equation}
\begin{aligned}
...
```{r}
#| label: SEM-estimate-BFI2-extraversion-models
library(tidyverse)
...
```{r}
#| label: fig-BFI2-extr-1-plot
#| fig-cap: Graphical representation of a single factor model for Extraversion. Items
#|   i1, i16, i31, and i46 relate to the Sociability facet of Extraversion, with items
#|   i16 and i31 stated in the negative direction. Items i6, i21, i36, and i51 relate
#|   to the Assertiveness facet of Extraversion, with items i36 and i51 stated in the
#|   negative direction. Items 11, 26, 41, 56 relate to the Energy Level facet of Extraversion,
#|   with items i11 and i26 stated in the negative direction.
#| fig-width: 8
#| fig-height: 3.5
#| out-width: 100%
...
@fig-BFI2-extr-1-plot shows the estimated parameters of a single factor model for the 12 items related to extraversion in the BFI-2. The factor loading of the first item was fixed to 1 in order to scale the latent factor. Note that all estimated factor loadings are positive, whilst 6 of the 12 items were stated in a negative direction (i.e. higher scores should indicate *less* extraversion). If all items relate to extraversion in the intended manner, we should expect a mix of positive and negative factor loadings. @tbl-BFI2-extr-1-results shows the parameter estimates and test results, as well as the model fit indices. As can be seen there, the single factor model is rejected by the overall model $\chi^2$ test, and the CFI, SRMR and RMSEA indicate relatively poor model fit.

```{r}
#| label: tbl-BFI2-extr-1-results
#| tbl-cap: "Results of a single factor model for extraversion"
...
tmp <- semoutput::sem_tables(BFI2_extr_1)
...
# cat(": Results of a single factor model for extraversion. {#tbl-BFI2-extr-1-results}\n\n")
...
```{r}
#| label: fig-BFI2-extr-1acq-plot
#| fig-cap: Graphical representation of a single factor model for Extraversion with an
#|   additional Acquiescence factor.
#| fig-width: 8
#| fig-height: 3.5
#| out-width: 100%
...
By fixing all the factor loadings, this additional factor only requires one additional parameter for the variance of the latent Acquiescence factor. The resulting model is depicted in @fig-BFI2-extr-1acq-plot and the resulting estimates and fit measures are provided in @tbl-BFI2-extr-1acq-results. Whilst there are now a few negative factor loadings and the fit is somewhat improved, the model is still not adequate. 

```{r}
#| label: tbl-BFI2-extr-1acq-results
#| tbl-cap: "Results of a single factor model for Extraversion with an additional Acquiescence factor"
...
tmp <- semoutput::sem_tables(BFI2_extr_1acq)
...
tmp <- semoutput::sem_tables(BFI2_extr_31acq)
...
#cat(": Results of a single factor model for Extraversion with an additional Acquiescence factor. {tbl-BFI2_extr_31acq}\n\n")
```
...
```{r}
#| label: SEM-estimate-BFI2-full-factor-models
mod_spec <- '
E =~ i1 + i16 + i31 + i46 + i6 + i21 + i36 + i51 + i11 + i26 + i41 + i56
...
```{r}
#| label: fig-BFI2-full-31acq-plot
#| fig-cap: Graphical representation of a hierarchical factor model for all personality
#|   domains and an additional Acquiescence factor.
#| fig-width: 16
#| fig-height: 4
#| out-width: 100%
...
```{r}
#| label: tbl-BFI2-cfa-hier-comparison
#| tbl-cap: "Fit measures for hierarchical factor models for the BFI-2 with an additional Acquiescence factor. The Independent model assumes all factors are independent. The Dependent model allows dependence between the 'big five' factors, but assumes these are independent from the Acquiescence factor"
...
tmp <- semoutput::sem_modelcomp(BFI2_cfa_hier, BFI2_cfa_hier2)
...
# cat(": Fit measures for hierarchical factor models for the BFI-2 with an additional Acquiescence factor. The Independent model assumes all factors are independent. The Dependent model allows dependence between the 'big five' factors, but assumes these are independent from the Acquiescence factor. {tbl-BFI2-cfa-hier-comparison}\n\n")
...
```{r}
#| label: tbl-BFI2-cfa-hier-results
#| tbl-cap: "Results of a hierarchical factor model for the BFI-2 with an additional Acquiescence factor"
...
tmp <- semoutput::sem_tables(BFI2_cfa_hier)
...
# cat(": Results of a hierarchical factor model for the BFI-2 with an additional Acquiescence factor. {tbl-BFI2-cfa-hier-results}\n\n")
```
