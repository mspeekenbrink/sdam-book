# A model of means (ANOVA) {#sec-ch-ANOVA}
...
```{r}
#| label: tetris-descriptives-table
library(dplyr)
dat <- tetris2015
...
Boxplots for each condition are provided in @fig-tetris-intrusive-memories.

```{r}
#| label: fig-tetris-intrusive-memories
#| fig-cap: Number of intrusive memories during days 1 to 7 for each condition of Experiment
#|   2 of James et al. (2015).
library(ggplot2)
...
Note that the model assumes that while the mean can differ between the conditions, deviations of the observations around the mean are assumed to have the same standard deviance $\sigma_\epsilon$. An example of what the model might look like is given in @fig-two-group-Normal-densities.

```{r}
#| label: fig-two-group-Normal-densities
#| fig-cap: Two Normal density functions with a different mean but identical standard
#|   deviation.
#| fig-height: 4
#| out-width: 80%
...
is formally equivalent to the model of @eq-two-group-model-mu. To see why, it is important to realize that linear regression concerns the conditional means of the dependent variable given the values of the predictor variable(s):
$$\hat{Y} = \mu_{Y|X_i}$$
...
For cases in the Tetris+Reactivation condition, the model predictions are a constant: $\beta_0$. If the errors are Normal-distributed, this is effectively the same model as we used in Chapter @sec-ch-simple-GLM. The only difference is in the label for the constant: in Chapter @sec-ch-simple-GLM we labelled this as $\mu$, and here we label it as $\beta_0$. Although the labels look different, they both refer to the same thing, namely the (conditional) mean of the dependent variable. If we "relabel" $\beta_0 = \mu_\text{t+r}$, then we have the model we wanted for this condition. 
...
```{r}
#| label: tetris-effect-coding-table
...
@fig-tetris-effect-coding-parameter-plot shows how the parameters of the model are related to the average number of intrusions in each condition.
```{r}
#| label: fig-tetris-effect-coding-parameter-plot
#| fig-cap: Average number of instrusions in the four conditions in the Tetris study
#|   and how they are related to the parameters of the effect-coding model. The intercept
#|   $\hat{\beta}_0 = \hat{\mu}$ is the grand mean (dotted line). The slopes reflect
#|   treatment effects, which are deviations from the average intrusions in a condition
#|   and the grand mean. The averages of the first three conditions are equal to $\beta_0
#|   + \beta_j$, the sum of the intercept and the slope of the effect-coding predictor
#|   $X_j$ representing the treatment effect of that condition. The mean of the last
#|   condition is the intercept minus the sum of the slopes of the effect-coding predictors.
...
@tbl-tetris-effect-coding-ANOVA also includes a row labelled "Condition". This is a test of the hypothesis that _all_ of the slopes of the contrast-coded predictors for Condition are equal to 0:
...
```{r}
#| label: tbl-tetris-orthogonal-contrast-codes
#| tbl-cap: "A set of orthogonal contrast codes."
...
Because the contrast codes $c_1$, $c_2$, and $c_3$ are orthogonal, we can use @eq-estimate-slope-orthogonal-contrast-codes to determine the estimates of the slopes. The estimated slope of the contrast-coded predictor $X_1$ which takes its values from $c_1$ will be:
$$
\begin{aligned}
...
```{r}
#| label: tetris-custom-coding-lm
contrasts(dat$Condition) <- codes
modg <- lm(intrusions ~ Condition, data=dat)
```
$$`r write_GLM_equation(modg, dv_name = "intrusions", iv_names = c("}X_1\\texttt{","}X_2\\texttt{","}X_3\\texttt{"), digits=3, include_sde = TRUE)`$$
As you can see, the parameter estimates are identical to those worked out above using @eq-estimate-slope-orthogonal-contrast-codes and the sample means. @fig-tetris-contrast-coding-parameter-plot shows how the parameters of the model are related to the average number of intrusions in each condition.
```{r}
#| label: fig-tetris-contrast-coding-parameter-plot
#| fig-cap: Average number of instrusions in the four conditions in the Tetris study
#|   and how they are related to the parameters of the orthogonal contrast-coding model.
#|   The intercept $\hat{\beta}_0 = \hat{\mu}$ is the grand mean (dotted line). The slopes
#|   reflect deviations from the average intrusions in a condition and combinations of
#|   other conditions.
data.frame(Condition = factor(1:4,labels=c("Control","Tetris+Reactivation","Tetris-Only","Reactivation-Only")), mean=means) %>%
...
```{r}
#| label: tetris-custom-coding-table-2
...
__Helmert coding__ involves a set of contrast codes in which each code compares a group mean to the average of all group means that come before it. You can find these contrast, for situations with a group size of $g=1, 2, \ldots, 6$, in @tbl-Helmert-contrasts. Using Helmert contrasts will always provide a set of orthogonal contrasts. And if you reorder the conditions in @tbl-tetris-orthogonal-contrast-codes as Tetris+Reactivation, Reactivation-Only, tetris-Only, Control, you should recognize that this was actually an example of Helmert coding for $g=4$. 

```{r}
#| label: tbl-Helmert-contrasts
#| tbl-cap: "Helmert contrast codes for situations with two to six groups"
options(knitr.kable.NA = "")
...
```{r}
#| label: tetris-contrast-codes-slopes
tab <- cbind(estimate = c(coefficients(modg)[1],NA,coefficients(modg)[2:4],Error=NA),rbind(car::Anova(modg, type=3)[1:2,],expand_Anova(modg, type=3)[-1,]))
```

As contrast-coded predictors are essentially just predictors in a linear model, we can use similar measures of effect-size as in multiple regression (see Section @sec-5-effect-size). So we can measure the size of an effect as the "proportion of variance explained", or "eta-squared"
...
For both these measures, the effect can be either a single contrast-coded predictor, or a set of these. So we may also consider the effect of Condition. In @tbl-tetris-contrast-coding-ANOVA, we can see that the Sum of Squares Reduced for Condition is $\text{SS}(\text{Condition}) = `r round(tab[2,2],2)`$. The value of $\text{SS}(\text{total})$ is `r round(sum(tab[c(2,6),2]),2)`, which is equal to the sum of $\text{SS}(\text{Condition})$ and $\text{SS}(\text{error})$. We can then compute the eta-squared as
...
The assumptions of the ANOVA model are the same as those for a (multiple) regression model (see Section @sec-5-assumptions). Thus, we assume the errors $\epsilon$ are independent and Normal-distributed, with a mean of 0 and a homoscedastic standard deviation $\sigma_\epsilon$. As an ANOVA model provides the same prediction for each case in a group/condition, the errors reflect variation in the dependent variable within each group or condition. Another way to state the assumptions of the General Linear Model in the case of an ANOVA model is then that the variance of the dependent variable within each group or condition is the same (see @fig-four-group-Normal-densities).

```{r}
#| label: fig-four-group-Normal-densities
#| fig-cap: An ANOVA model assumes that the variance within each condition or group is
#|   identical. The means of the conditions or groups can differ.
#| fig-height: 4
#| out-width: 80%
mus <- c(3.5,4,4.2,6)
...
$$F_\text{Scheffé} = (g-1)F_{g-1,n-\text{npar}(G);\alpha_\text{FW}}$$
...
4. Report the results. When reporting the results, make sure that you include all relevant statistics. For example, one way to write the results of the analysis with the contrast codes of @tbl-tetris-orthogonal-contrast-codes, is as follows:
...
The assumptions of the oneway ANOVA model are the same as the assumptions of any General Linear Model (see Section @sec-04b-assumptions). Thus, we assume the errors (residuals) of the model to be Normal-distributed with a mean of 0, a constant standard deviation $\sigma_\epsilon$, and each error term $\epsilon_i$ to be independent from any other one. However, because a oneway ANOVA makes the same prediction within each group, $\hat{Y} = \overline{Y}_j$, where $\overline{Y}_j$ is the sample mean of the dependent variable in the group the case belongs to.  
-->
